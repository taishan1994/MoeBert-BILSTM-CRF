...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        # )
        layer_output, aux_loss  = self.moe(attention_output)

        outputs = (layer_output,) + outputs + (aux_loss, )
    ...
    def forward(
        ...
        if not return_dict:
            return tuple(
                ...
                if v is not None
            ) + (torch.stack(all_aux_losses).sum(),)
        ...
        outputs.aux_loss = torch.stack(all_aux_losses).sum()  # 动态添加属性
        return outputs
    ...
    def forward(
        ...
        r"""
            labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
                ...
                the loss is only computed for the tokens with labels in `[0, transformers., config.vocab_size]`
...
class BertLMHeadModel(BertPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        labels (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            ...
            `[-100, 0, transformers., config.vocab_size]` (see `input_ids` docstring) Tokens with indices set to `-100` are
            ignored (masked), the loss is only computed for the tokens with labels n `[0, transformers., config.vocab_size]`
        past_key_values (`tuple(tuple(torch.FloatTensor))` of length `config.n_layers` with each tuple having 4 tensors of shape `(batch_size, num_heads, sequence_length - 1, embed_size_per_head)`):
            Contains precomputed key and value hidden states of the attention blocks. Can be used to speed up decoding.
