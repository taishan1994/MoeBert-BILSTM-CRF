...
from transformers import BertTokenizer, TrainingArguments, AutoTokenizer
from torchcrf import CRF
from transformers import Trainer, AdamW, get_linear_schedule_with_warmup
from modeling_moebert import BertModel
from transformers import BertModel as StandardBertModel
from configuration_moebert import BertConfig
...
class BertNer(nn.Module):
    def __init__(self,
        ...
        self.bert = StandardBertModel.from_pretrained(bert_dir)
        self.bert_config = BertConfig.from_pretrained(bert_dir)
        self.bert = self.convert_bert2moe()
    ...
    def convert_bert2moe(self):
        moe_model = BertModel(config=self.bert_config)
        # 2. 复制非MoE部分的权重 (Embeddings, Pooler)
        # 使用deepcopy来确保不共享内存
        moe_model.embeddings = deepcopy(self.bert.embeddings)
        if self.bert.pooler is not None:
            moe_model.pooler = deepcopy(self.bert.pooler)

        # 3. 逐层复制Encoder的权重
        for i, standard_layer in enumerate(self.bert.encoder.layer):
            moe_layer = moe_model.encoder.layer[i]

            # 3.1 复制自注意力部分
            moe_layer.attention = deepcopy(standard_layer.attention)

            # 3.2 复制交叉注意力部分（如果存在）
            if standard_layer.add_cross_attention:
                moe_layer.crossattention = deepcopy(standard_layer.crossattention)

            # 3.3 === 核心：权重克隆 ===
            # 将标准FFN的权重克隆到MoE层的所有专家中
            original_intermediate = standard_layer.intermediate
            original_output = standard_layer.output

            for expert in moe_layer.moe.experts:
                # 复制 intermediate 层
                expert.intermediate.dense.weight.data.copy_(original_intermediate.dense.weight.data)
                expert.intermediate.dense.bias.data.copy_(original_intermediate.dense.bias.data)

                # 复制 output 层 (包括dense和LayerNorm)
                expert.output.dense.weight.data.copy_(original_output.dense.weight.data)
                expert.output.dense.bias.data.copy_(original_output.dense.bias.data)
                expert.output.LayerNorm.weight.data.copy_(original_output.LayerNorm.weight.data)
                expert.output.LayerNorm.bias.data.copy_(original_output.LayerNorm.bias.data)

        # 4. 门控网络(moe_layer.moe.gate)保持随机初始化，它将在训练中学习

        print("成功将标准BERT模型转换为MoE架构并初始化权重！")
        return moe_model
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        loss = None
        all_loss = None
        aux_losses = seq_out[-1]
        if labels is not None:
            loss = -self.crf(seq_out, labels, mask=attention_mask.bool(), reduction='mean')
            all_loss = loss + aux_losses
        return all_loss, {"logits": logits, "labels": labels}
